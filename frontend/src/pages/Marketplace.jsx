...
export default function Marketplace({ user }) {
  ...
  return (
    <div>
      {/* Header left-aligned per §1.3 */}
      <div style={{marginBottom:24}}>
        <h1 style={{fontSize:32, marginBottom:8}}>Marketplace</h1>
        <p style={{color:'var(--ink-60)', fontSize:14, maxWidth:'70ch'}}>Fractional ownership · Fixed-supply tokens · Instant finality on Drunix. Progress indicator shows % tokens sold — liquidity and demand at a glance per §4.1</p>
      </div>
      ...
      <div className="card" style={{padding:'16px 20px', marginBottom:24, display:'flex', gap:12, flexWrap:'wrap', alignItems:'center'}}>
        ...
        <div style={{marginLeft:'auto', display:'flex', gap:8, alignItems:'center'}}>
          <span style={{fontSize:11, color:'var(--ink-40)'}}>{filtered.length} properties · Index: idx_property_status</span>
          <button className="btn btn-secondary" style={{padding:'8px 12px', fontSize:12}} onClick={fetchProperties}>Refresh</button>
        </div>
      </div>

      {loading ? <p style={{color:'var(--ink-60)', fontSize:14}}>Loading properties from SQL state store...</p> : (
              ...
              <div key={prop.assetId} className="card" style={{display:'flex', flexDirection:'column', gap:16, padding:20}}>
                ...
                <div style={{fontSize:12, color:'var(--ink-60)', lineHeight:1.6}}>
                  ...
                  <div style={{marginTop:8, display:'flex', gap:16, flexWrap:'wrap'}}>
                    <span className="tabular" style={{fontWeight:600, color:'var(--ink)'}}>₹{valuationLakh}L valuation</span>
                    <span className="tabular">{prop.totalTokens || '—'} tokens</span>
                  </div>
                  {tokenPrice > 0 && (
                    <div style={{marginTop:8, display:'flex', gap:8, alignItems:'center'}}>
                      ...
                      <span style={{fontSize:10, background:'var(--paper)', border:'1px solid var(--ink-8)', padding:'2px 6px', borderRadius:4, color:'var(--ink-40)'}}>1 token = ₹{tokenPrice}</span>
                    </div>
                  )}
                </div>

                {/* Progress indicator per §4.1 */}
                <div>
                  <div style={{display:'flex', justifyContent:'space-between', fontSize:11, marginBottom:6}}>
                    <span style={{color:'var(--ink-60)'}}>{soldPct}% sold</span>
                    <span className="tabular" style={{color:'var(--ink-60)'}}>{prop.soldTokens || Math.floor(prop.totalTokens*soldPct/100)}/{prop.totalTokens} tokens</span>
                  </div>
                ...
                <div style={{display:'flex', gap:8, marginTop:'auto', alignItems:'center'}}>
                  <Link to={`/property/${prop.assetId}`} className="btn btn-primary" style={{textDecoration:'none', fontSize:13, flex:1, justifyContent:'center'}}>View property →</Link>
                  <span style={{fontSize:10, color:'var(--ink-40)'}}>{prop.originatorId}</span>
                </div>

                <div style={{fontSize:10, color:'var(--ink-40)', borderTop:'1px solid var(--ink-8)', paddingTop:8, fontFamily:'ui-monospace, monospace'}}>
                  balance~{prop.assetId.slice(0,12)}~ownerId · SQL idx
                </div>
              </div>
            )
          })}
      ...
      <div className="card" style={{marginTop:24, background:'var(--paper)', borderStyle:'dashed'}}>
        <h4 style={{fontSize:12, fontWeight:700, letterSpacing:'0.06em', textTransform:'uppercase', color:'var(--registry-navy)', marginBottom:8}}>Drunix SQL State Store Advantage</h4>
        <p style={{fontSize:12, color:'var(--ink-60)', lineHeight:'1.6', maxWidth:'80ch'}}>
          Unlike vanilla Fabric's LevelDB/CouchDB, Drunix exposes on-chain SQL. Marketplace queries use <code>idx_property_status</code> and wallet queries use <code>idx_balance_owner</code> for O(log n) lookups, avoiding full ledger scans. Cap-table per asset via <code>idx_balance_asset</code>. Regulator audit via <code>idx_transfer_asset_time</code>.
        </p>
      </div>
    </div>
  )
