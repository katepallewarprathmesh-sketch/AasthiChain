...
export default function PropertyDetail({ user }) {
  ...
  const fetchDetails = async () => {
    ...
    try {
      ...
      if (!propRes.ok) {
        // Try to parse error, but don't show raw HTML to user
        let errMsg = `Property not found (${propRes.status})`
        try {
          const errJson = await propRes.json()
          errMsg = errJson.error || errMsg
        } catch {
          // If response is HTML (Vercel rewrites issue), treat as not found and use fallback
      ...
      if (e.name === 'AbortError') {
        ...
        // Don't show technical JSON parse errors to user — use friendly fallback
        if (e.message.includes('<!DOCTYPE') || e.message.includes('Unexpected token')) {
          setError('') // Hide technical error, show demo data
        } else {
          setError(e.message)
        }
      }
      // Fallback demo data so UI never blank
      if (!property) {
        setProperty({
          assetId: id,
        ...
        <div className="card">
          ...
          <div style={{marginTop:16}}>
            ...
            <div style={{marginTop:20}}>
              <button className="btn btn-primary" style={{width:'100%', padding:'12px', fontSize:14, fontWeight:600}}>Buy Tokens</button>
            </div>
